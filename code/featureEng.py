...
def systemGenFeatureEng(df, prevSteps, nextSteps, blockDiffs=False):
    """
    ...
    """
    if type(prevSteps) == list: # multiple blocks
        prev = []
        for block in prevSteps:
            pTmp = getTimeSteps(df, block)
            if blockDiffs:
                pTmp = dataDiff(pTmp) 
            prev.append(pTmp)
    else: # single block specified as a numpy array
        prev = [getTimeSteps(df, prevSteps)]
        if blockDiffs:
            prev = dataDiff(prev) 

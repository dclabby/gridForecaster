...
def tuneModel(xTrain, yTrain, model, modelParams, cvSplit):
    ...
    gsearch = GridSearchCV(estimator=model, cv=tscv, param_grid=modelParams, scoring = 'r2', verbose=3) # https://scikit-learn.org/stable/modules/model_evaluation.html#the-scoring-parameter-defining-model-evaluation-rules
...
def trainModels(xTrain, yTrain, modelsToTrain, cvSplit=5):        
    ...
    for modelName, modelParams in modelsToTrain.items():
        ...
        if len(modelParams) == 0: # no parameters specified so the models are trained based on default parameters
            ...
            # if one parameter defined as list and others not defined as list then GridSearchCV will throw an error. THerefore check if any other parameters are not lists and convert if necessary
            notLists = [k for k, v in modelParams.items() if type(v) != list]
            if len(notLists) > 0:
                print("warning: " + str(notLists) + " must be defined as lists for input to GridSearchCV... Parameters will be converted to lists.")
                for k in notLists:
                    modelParams[k] = [modelParams[k]]            
